...
Strategic Goal: {initiative.strategicGoal}
Owner: {initiative.owner}
{initiative.department && Department: {initiative.department}}
Timeline: {initiative.timeline}
Status: <span className={inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${ initiative.status === 'On Track' ? 'bg-green-100 text-green-800' : initiative.status === 'At Risk' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800' }}>{initiative.status}

Major Milestones (Summary):

{initiative.milestones.map((m, idx) => {m})}
...
{@media print { /* Hide elements not needed in print snapshot */ #filters-section, #initiatives-table-section, .no-print { display: none !important; } /* Adjust header for print */ header { padding: 1rem !important; margin-bottom: 1rem !important; } header h1 { font-size: 1.5rem !important; } /* Ensure other sections are visible and layout correctly */ #kpi-summary-section, #priority-cards-section { display: block !important; page-break-after: auto; } #priority-cards-section .grid { grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)) !important; gap: 1rem !important; } /* Adjust card content for readability on print */ #priority-cards-section h3 { font-size: 1rem !important; } #priority-cards-section p { font-size: 0.75rem !important; } }}
